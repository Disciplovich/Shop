<template>
  <div class="form-container">
    <h2 class="title">✏ Редактировать магазин</h2>
...
export default {
  ...
  computed: {
    ...mapGetters(['selectedShop']),
    shop() {
      return this.selectedShop || {};
    },
    formattedDate: {
      get() {
        if (!this.shop.openingDate) return '';
        return this.shop.openingDate.split('T')[0];
      },
      set(value) {
        this.shop.openingDate = value;
      }
    }
  },
  async created() {
    await this.fetchShop(this.$route.params.id);
    this.loading = false;
  },
  methods: {
    ...mapActions(['fetchShop', 'updateShop']),
    async submitForm() {
      try {
